                            ...
                            Using cmd2 As New OleDbCommand(sql2, conn)
                                ...
                                Using dr2 As OleDbDataReader = cmd2.ExecuteReader
                                    ...
                                    If Not dr2.Read() Then
                                        '加入新帳號
                                        Dim sql3 As String = "insert into ControllerSetup(Account,ECO_Account,ECO_Password,ECO_Type,CtrlNr,DrawNr,InstallPosition,IP_Address,Enabled,SMSSentEnabled,EmailSentEnabled,Num,Mail,DayMailSentEnabled,MonthMailSentEnabled,DayMail,MonthMail,DiffTime) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
                                        Using cmd3 As New OleDbCommand(sql3, conn)
                                            ...
                                            '    cmd3.Parameters.AddWithValue("?WeekMailSnetEnabled", True)
                                            'Else
                                            '    cmd3.Parameters.AddWithValue("?WeekMailSnetEnabled", False)
                                            'End If
                                            ...
                                            End If
                                            cmd3.Parameters.AddWithValue("?DayMail", daymail_txt.Text)
                                            'cmd3.Parameters.AddWithValue("?WeekMail", weekmail_txt.Text)

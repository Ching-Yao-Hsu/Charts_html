...
/****** Object:  StoredProcedure [dbo].[ReadMeterTree]    Script Date: 2016/12/1 下午 03:17:17 ******/
SET ANSI_NULLS ON
GO
				   ...
				   if (@sCtrlNr <> '' and @sMeterID <> '')
					  begin
						 Select CS.Account as Account, CS.InstallPosition AS ECO_Position, CS.Enabled as ECO_Enabled, 
								...
								,DateDiff(n, isnull(MS.recdate,'') , ( CONVERT(NVARCHAR, dateAdd(hh,ISNULL(CS.DiffTime ,0),getdate()), 111) + ' '  + CONVERT(NVARCHAR, dateAdd(hh,ISNULL(CS.DiffTime ,0),getdate()), 108))) as UpLoadStatus 

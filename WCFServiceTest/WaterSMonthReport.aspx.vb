...
Partial Class WaterSMonthReport
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Not IsPostBack Then
                sMonth = Request.QueryString("sMonthS")
            Else
                sMonth = CYMD(txtMonth.Text)
            End If
            Label1.Text = "月份：" & sMonth
            txtMonth.Text = sMonth
            Label1.Visible = False
            SqlQuery(sMonth, sShopNo, sDrawNo, sShopName)
        End If
    End Sub

    Protected Sub SqlQuery(ByVal sMonth As String, ByVal sShopNo As String, ByVal sDrawNo As String, ByVal sShopName As String)
        ...
        Try
            ...
            Dim sql As String = " Select isNull(Round(Sum(Amount),4),0) as SumAmount From Record Where SubString(Recdate, 1,7)  = '" & sMonth & "' " & _
            ...
            Dim strSQL As String = " Select Substring(RecDate, 6, 5) as 日期 , Round(Sum(Amount),4) 用水量 , '' as 備註 " & _
                                   " From Record With (Nolock) " & _
                                   " Where ID in (Select ID From Setup with (Nolock) Where 店鋪編號='" & sShopNo & "' and 圖面編號='" & sDrawNo & "' and 店鋪名稱='" & sShopName & "') " & _
                                   " And SubString(Recdate, 1,7)  = '" & sMonth & "' " & _
                                   " Group by Substring(RecDate, 6, 5)  Order by 1 "
    ...
    Protected Sub print_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        SqlQuery(txtMonth.Text, LabShopNo.Text, LabDrawNo.Text, LabShopName.Text)
    ...
    Protected Sub Excel_Click(sender As Object, e As ImageClickEventArgs) Handles Excel.Click
        ...
        sFilename = Server.UrlPathEncode("月報表_" & Now())
        ...
        Label1.Visible = True : Label5.Visible = False : txtMonth.Visible = False : ButtonSM.Visible = False
        ...
        Label1.Visible = False : Label5.Visible = True : txtMonth.Visible = True : ButtonSM.Visible = True
        Response.End()
    End Sub

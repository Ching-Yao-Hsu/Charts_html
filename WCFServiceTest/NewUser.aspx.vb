...
Partial Class _Default
        ...
        Else
            If Not Page.IsPostBack Then
                ...
                Using conn As New OleDbConnection(strcon)
                    ...
                    Dim strSQL As String = " Select * from  [dbo].[AdminSetup] where Account in (Select Com From AdminCom Where Account='" & Session("Account") & "' and Enabled=1) and Enabled = 1 and CreateDB = 1 and Rank <> 0 and PerNum='" & Session("PerNum") & "' and account<>'admin'"
                ...
                Using dr1 As OleDbDataReader = cmd1.ExecuteReader
                    If Not dr1.Read() Then
                        Try
                            ...
                            If strPerNum = "1" Then
                                If Session("PerNum") <> 0 Then
                                    strSQL2 = strSQL2 & " and Account in (Select Account from AdminSetup where PerNum='" & Session("PerNum").ToString.Trim & "' And Account <>'" & account_txt.Text & "') "
                                End If
                            End If

                            strSQL2 = strSQL2 & "; Insert into AdminCom Select '" & account_txt.Text & "' as Account, Com , 0 Enabled , getdate() as EnabledTime From AdminCom with (Nolock) Where account='admin' and com<>'admin' "  '以Admin的權限來復製
                            If strPerNum = "1" Then
                                If Session("PerNum") <> 0 Then
                                    strSQL2 = strSQL2 & " and Com in (Select Account from AdminSetup where PerNum='" & Session("PerNum").ToString.Trim & "' And Account <>'" & account_txt.Text & "') "
                                End If
                            End If
                            ...
                            Using cmd2 As New OleDbCommand(strSQL2, conn)
                                ...
                                If CreateDB_CB.Checked = True Then    '//建立資料庫
                                    CreateDB(account_txt.Text)
                                End If
                            End Using
    ...
    Protected Sub CreateDB(ByVal account As String)
        Try
            ...
            Dim sr As StreamReader = New StreamReader(Server.MapPath("./SQL/") + "Master_ECOProcdureSTD.sql", System.Text.Encoding.Default)
